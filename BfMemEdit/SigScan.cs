...
namespace BfMemEdit
{
    public class SigScan
    {
        ...
        public List<IntPtr> FindAllPattern(byte[] btPattern, string strMask, int nOffset)
        {
            try
            {
                ...
                for (int x = 0; x < this.m_vDumpedRegion.Length; x++)
                {
                    if (this.MaskCheck(x, btPattern, strMask))
                    {
                        ...
                        found.Add(new IntPtr((int)this.m_vAddress + (x + nOffset)));
                    }
                }
        ...
        ///     is added to the located address and returned to the user.
        /// </summary>
        /// <param name="strPattern">String pattern to look for in the dumped region.</param>
        /// <param name="strMask">The mask string to compare against.</param>
        /// <param name="nOffset">The offset added to the result address.</param>
        /// <returns>IntPtr - zero if not found, address if found.</returns>
        public IntPtr FindPattern(string strPattern, string strMask, int nOffSet)
        {
            if (string.IsNullOrWhiteSpace(strMask))
            {
                strMask = new string('x', strPattern.Length);
            }
            return FindPattern(Encoding.UTF8.GetBytes(strPattern), strMask, nOffSet);
        }
        ...
        public void FindPattern()
        {
            try
            {
                ...
                for (int x = 0; x < this.m_vDumpedRegion.Length; x++)
                {
                    ...
                    {
                        if (this.MaskCheck(x, sig.Pattern, sig.Mask))
                        {
                            // The pattern was found, set it.
                            sig.Address = this.m_vAddress + (x + sig.Offset);
                        }
                    }
                }
